...
UnloggedTableName = 'CensusDataTableUnlogged'
...
def getSQLcmnds(rowlist):
	...
	for row in rowlist:
		valstr = row2vals(row)
		cmd = f"INSERT INTO {UnloggedTableName} VALUES ({valstr});"
		cmdlist.append(cmd)
	return cmdlist
...
def createUnloggedTable(conn):
	...
	with conn.cursor() as cursor:
		cursor.execute(f"""
        	DROP TABLE IF EXISTS {UnloggedTableName};
        	CREATE UNLOGGED TABLE {UnloggedTableName} (
            	Year                INTEGER,
              CensusTract         NUMERIC,
            	State               TEXT,
            	County              TEXT,
	...
	with conn.cursor() as cursor:
		...
		for cmd in icmdlist:
			...
			cursor.execute(cmd)
                ...
                cursor.execute(f"""INSERT INTO {UnloggedTableName} SELECT * FROM {TableName};""")
